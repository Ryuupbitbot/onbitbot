...
def macd_min1(coin):
    ...
    if (prev_close < ma60[-2] or prev2_close < ma60[-3] or prev3_close < ma60[-4]) and df['close'][-1] > ma60[-1] and ma30[-1] > ma30[-2] and ma30[-1] > ma60[-1]:
...
def buy_test3(coin):
        for coin in coins:
            if (coin in list_1 or coin in list_5) and coin in list_10 and coin in list_13:
    ...
    try:
        ...
        for coin in list(buy_waitlist1):
            ...
            if coin not in buy_waitlist:
                ...
                post_message(myToken,"#upbit1", coin+" buy waiting list at by-1: " +str(current_price))
        ...
        for coin in list(buy_waitlist3):
            ...
            if coin not in buy_waitlist:
                buy_waitlist[coin] = {'price': current_price, 'time': time.time()}
                print(f"{coin} Buy Waiting List in {current_price}")
                post_message(myToken,"#upbit1", coin+" buy waiting list at by-3 : " +str(current_price))
                krw_balance = upbit.get_balance("KRW")
                if krw_balance is not None and krw_balance > invest_money1*1.01:
                    order = upbit.buy_market_order(coin, invest_money1)
                    sell_waitlist[coin] = {'price': current_price, 'time': time.time()}

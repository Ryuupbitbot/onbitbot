...
# Initialize lists to keep track of pending sales and manual sell orders
pending_sales = []
manual_sell_orders = []
...
def check_interest_coin(access_key, secret_key, coin):
    # Set up Upbit API client
    upbit = pyupbit.Upbit(access_key, secret_key)

    if coin in my_coins:
        # Get the quantity of the coin in your account and the average purchase price
        coin_balance = upbit.get_balance(coin)
        coin_avg_price = upbit.get_avg_buy_price(coin)

        # Get the available-for-sale quantity of the coin
        coin_sellable_balance = upbit.get_balance(coin.split('-')[1])

        return (coin_balance, coin_sellable_balance, coin_avg_price)

    else:
        return None

def sell_on_profit(access_key, secret_key, coin):
    # Set up Upbit API client
    upbit = pyupbit.Upbit(access_key, secret_key)

    # Get the current price of the coin and the maximum price in the past 24 hours
    current_price = pyupbit.get_current_price(coin)
    max_price = pyupbit.get_ohlcv(coin, interval='day')['high'].max()

    # Get the quantity of the coin in your account and the average purchase price
    coin_balance, _, coin_avg_price = check_interest_coin(access_key, secret_key, coin)
    coin_sellable_balance = upbit.get_balance(coin.split('-')[1])

    if coin_balance is not None:
        # Calculate the profit rate
        profit_rate = (current_price / coin_avg_price - 1) * 100

        if profit_rate > 5 and current_price < max_price * 0.986:
            # Sell the available-for-sale quantity at the market price
            upbit.sell_market_order(coin, coin_sellable_balance)

def sell_on_loss(access_key, secret_key, coin):
    # Set up Upbit API client
    upbit = pyupbit.Upbit(access_key, secret_key)
    ...
    # Get the quantity of the coin in your account and the average purchase price
    coin_balance, coin_sellable_balance, coin_avg_price = check_interest_coin(access_key, secret_key, coin)

    if coin_balance is not None:
        # Calculate the loss rate
        loss_rate = (current_price / coin_avg_price - 1) * 100
...
while True:
    ...
    try:
        ...
        def sell_on_profit(access_key, secret_key, coin):
            ...
            # Get the current price of the coin and the maximum price in the past 24 hours
            current_price = pyupbit.get_current_price(coin)
            max_price = pyupbit.get_ohlcv(coin, interval='day', count=3)['high'].max()
            ...
            if coin_balance is not None:
                # Calculate the profit rate
                profit_rate = current_price / coin_avg_price 

                if profit_rate > goodsell_percent and current_price < max_price * aftergoodsell_percent:
                    # Sell the available-for-sale quantity at the market price
                    upbit.sell_market_order(coin, coin_sellable_balance)
                    print(f"{coin} good sell {current_price}")
        ...
        def sell_on_loss(access_key, secret_key, coin):
            ...
            if coin_balance is not None:
                # Calculate the loss rate
                loss_rate = (current_price / coin_avg_price)

                if loss_rate < deadsell_percent:
                    # Sell the available-for-sale quantity at the market price
                    upbit.sell_market_order(coin, coin_sellable_balance)
                    print(f"{coin} dead sell {current_price}")

...
# Define the function to check the macd 1min indicator
def macd_min1(coin):
    data = pyupbit.get_ohlcv(coin, interval="minute1")
    if data is None:
        return False

    ma120 = talib.SMA(data['close'], timeperiod=120)
    ma120_trend = talib.EMA(ma120, timeperiod=10)
    ma120_trend_slope = talib.LINEARREG_SLOPE(ma120_trend, timeperiod=10)
    if ma120_trend_slope[-1] > 0:
        current_price = pyupbit.get_current_price(coin)
        previous_price = data['close'][-2]
        pprevious_price = data['close'][-3]
        if current_price > ma120[-1]:
            if previous_price < ma120[-2] or pprevious_price < ma120[-3]:
                return True
    return False
...
while True:
    try:
        ...
        post_message(myToken,"#upbit1", coin+" error while :" , (e))
...
###############################################################################
    try:
        ...
        print(f"Error_B: {e}")
        post_message(myToken,"#upbit1", coin+" error buy : ",(e))
...
################################################################################
    ...
    try:
        ...
        print(f"Error4: {e}")
        post_message(myToken,"#upbit1", coin+" error sell : " , (e))

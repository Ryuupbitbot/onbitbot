...
buydown_percent = 1.00
aftergoodsell_percent = 0.985 #goodsell각 이후 1.5% 떨어지면 매도
buy_waiting_time = 14400 #조건만족후 구매대기 최대 시간(초)
...
def macd_min1(coin):
    ...
    if (prev_close < ma60[-2] or prev2_close < ma60[-3] or prev3_close < ma60[-4]) and df['close'][-1] > ma60[-1] and ma30[-1] > ma30[-2]:
...
while True:
        ...
        for coin in buy_waitlist1:
            ...
            # Check if the buy order can be executed
                ...
                if current_price < temp_price * buydown_percent:
                    ...
                    del buy_waitlist[coin]
                    print(f"{coin} removed from the buy waiting list")
                    post_message(myToken,"#upbit1", coin+" removed from the buy waiting list" +str(current_price))
    ...
    try:
        ...
        for coin in coins:
            ...
            if balance is not None and avg_buy_price is not None:
                if balance > 0:
                    if (current_price >= goodsell_percent * avg_buy_price and current_price <= aftergoodsell_percent * three_day_high):
                    ...
                    elif (current_price <= deadsell_percent * avg_buy_price):
                        upbit.sell_market_order(coin, balance)

...
def macd30m(symbol):
    ...
    condition = False
    if (
        ((macd[2] - exp3[2] < 0) and (macd[1] - exp3[1] >= 0))
        or ((macd[1] - exp3[1] < 0) and (macd[0] - exp3[0] >= 0))
    ):
        condition = True
...
def buy_test (symbol):
    test = False
    if(
        macd60m(symbol) and macd30m(symbol) and stockrsiweeks(symbol) and stockrsidays(symbol) 
...
########## Buy ########
    try:
        ...
        common_coins = find_common_coins(krw_tickers, krw_tickers1)
        
        # Iterate over the list of tradable coins
        for symbol in common_coins:

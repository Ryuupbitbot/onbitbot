...
my_coins = ['KRW-BTC', 'KRW-ETH', 'KRW-NEO', 'KRW-MTL', 'KRW-XRP', 'KRW-ETC', 'KRW-OMG', 'KRW-SNT', 'KRW-WAVES',
              'KRW-XEM', 'KRW-QTUM', 'KRW-LSK', 'KRW-STEEM', 'KRW-XLM', 'KRW-ARDR', 'KRW-ARK', 'KRW-STORJ', 'KRW-GRS',
               ...
               'KRW-DAWN', 'KRW-AXS', 'KRW-STX', 'KRW-XEC', 'KRW-SOL', 'KRW-ARB', 'KRW-AAVE', 'KRW-APT', 'KRW-MASK',
               'KRW-NEAR', 'KRW-MATIC']
...
list_8 = [] # 240 min OBV (signal rising)
list_9 = [] # 30 min OBV (signal rising)
...
def envelope_day(coin):
    ...
    if (df['close'][-2] < lower[-2] and df['close'][-1] > lower[-1]) or (df['close'][-3] < lower[-3] and df['close'][-2] > lower[-2]):
        if df['close'][-3] < df['close'][-2] and df['close'][-2] < df['close'][-1]:
...
def buy_test1(coin):
        for coin in coins:
            if coin in list_1 or coin in list_11:
                    if coin in list_2:
                        if coin in list_3:
                            if coin in list_4:
                                if coin in list_5 or coin in list_8 or coin in list_11:
                                    if coin in list_6:
                                        if coin in list_7 or coin in list_12:
                                            if coin in list_9:
...
def buy_test3(coin):
        for coin in coins:
            if coin in list_1 or coin in list_2:
                if coin in list_5:
                    if coin in list_8:
                        if coin in list_7 or coin in list_12:
                            if coin in list_10:
...
while True:
    try:
        ...
        time.sleep(1)
        post_message(myToken,"#upbit1", "Error condition-1 : " + str(e))
    ...
    try:
        ...
        # Increment cycle_count and reset List if necessary
        cycle_count += 1
        if cycle_count == 9:
            list_1.clear()
            list_2.clear()
